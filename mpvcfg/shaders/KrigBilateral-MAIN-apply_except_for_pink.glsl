...
vec4 hook() {
    // Calculate bounds
    float low  = ceil((MAIN_pos - OLDCHROMA_pt) * MAIN_size - offset - 0.5)[axis];
    float high = floor((MAIN_pos + OLDCHROMA_pt) * MAIN_size - offset - 0.5)[axis];
    ...
    for (float k = low; k <= high; k++) {
        pos[axis] = MAIN_pt[axis] * (k - offset[axis] + 0.5);
        float rel = (pos[axis] - MAIN_pos[axis])*OLDCHROMA_size[axis];
        float w = Kernel(rel);

        vec4 y = dot(
            textureGrad(MAIN_raw, pos, vec2(0.0), vec2(0.0)).rgb,
...
vec4 hook() {
    ...
    float y = (mat3(0.2126,-0.09991,0.615,0.7152,-0.33609,-0.55861,0.0722,0.436,-0.05639) * MAIN_texOff(0).rgb).x;
    float Mx[(N*(N+1))/2];
    float b[N];
    vec2 interp = X[N].zw;

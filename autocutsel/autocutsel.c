...
static XrmOptionDescRec optionDesc[] = {
  ...
  {"-raw",       "raw",       XrmoptionNoArg,  "on"},
  {"-rawhex",    "rawhex",    XrmoptionNoArg,  "on"},
...
int Syntax(char *call)
{
  fprintf (stderr,
    ...
    " [-pause <milliseconds>] [-debug] [-verbose] [-raw] [-rawhex] [-fork] [-buttonup]\n",
    call);
  exit (1);
}
...
static XtResource resources[] = {
  ...
  {"raw", "Raw", XtRString, sizeof(String),
    Offset(raw_option), XtRString, "off"},
  {"rawhex", "RawHex", XtRString, sizeof(String),
    Offset(rawhex_option), XtRString, "off"},
...
/*        char *valueEscaped = (char *)malloc(options.length*2+1);
        char *p = valueEscaped;
...
char *value = options.value;
  for (int length = options.length; length > 0; length--, value++) {
...
printf("\0");
      ...
      else if (options.rawhex) {
        /*for (int i = 0; i < options.length; i++)
        {
          printf("%02x", (unsigned char)options.value[i]);
        }*/
        PrintHexValue(options.value, options.length);
  //      printf("\n");
printf("\n\0");
      }
      if (options.debug)
...
int main(int argc, char* argv[])
{
  ...
  if (strcmp(options.raw_option, "on") == 0)
    options.raw = 1;
  else
    options.raw = 0;

  if (strcmp(options.rawhex_option, "on") == 0)
    options.rawhex = 1;
  else
    options.rawhex = 0;
